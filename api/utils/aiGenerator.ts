  ...
  try {
    if (isAIAvailable()) {
      ...
      if (existingNames.length > 0) {
        prompt = `你是一位网文角色设计师。以下小说的标题、简介、分卷框架和世界观已全部生成完毕，请基于这些已生成的所有内容查漏补缺，补充缺失的角色：
...
【重要】查漏补缺规则：
1. 基于以上已生成的所有内容，找出尚未在已有角色列表中的角色名，为这些角色生成设定
2. 角色信息必须基于已有内容推断，不得脱离原文编造
3. 根据每个角色在故事中的核心程度判断定位：故事核心人物为"主角"，对立面为"反派"，重要但非核心为"配角"，次要为"龙套"
4. 如果已有角色加上新识别的角色仍不足3个，再根据剧情需要补充角色，保证总共至少3个角色
5. 补充的角色也必须符合已有内容的风格和设定

请返回JSON数组，每个角色包含：name, role(主角/配角/反派/龙套), age, appearance, personality, background, motivation, relationship`;
      } else {
        prompt = `你是一位网文角色设计师。以下小说的标题、简介、分卷框架和世界观已全部生成完毕，请基于这些已生成的所有内容识别并生成角色：
...
要求：
1. 【识别角色】基于以上已生成的所有内容，找出其中提到的所有角色名，不能遗漏
2. 【判断类型】根据每个角色在故事中的定位判断类型：故事核心人物为"主角"，对立面为"反派"，重要但非核心为"配角"，次要为"龙套"
3. 【生成设定】为每个识别到的角色生成完整设定，角色信息必须基于已有内容推断，不得脱离原文编造
4. 如果识别到的角色不足3个，再根据剧情需要补充角色，保证至少3个角色
5. 补充的角色也必须符合已有内容的风格和设定
